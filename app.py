...
# ============================================================
# OPENCV GRID DETECTION & OCR PROCESSING
# ============================================================
...
def detect_vertical_lines(image):
    ...
    for x in candidates:
        if not filtered or x - filtered[-1] >= min_space:
            filtered.append(x)
...
def find_row_centers(image, x_bounds):
    ...
    centers = []
    try:
        data = pytesseract.image_to_data(processed, config="--oem 3 --psm 11", output_type=Output.DATAFRAME)
        if data is not None and not data.empty:
            for _, item in data.dropna(subset=["text"]).iterrows():
                text = clean_text(item["text"])
                if len(text) >= 4:
                    cy = top + int(item["top"] / 3) + max(1, int(item["height"] / 3)) // 2
                    centers.append({"cy": cy, "conf": float(item.get("conf", 0))})
    except Exception:
        pass
    
    centers = sorted(centers, key=lambda x: x["cy"])
    unique_centers = []
    for item in centers:
        dup_idx = next((i for i, ex in enumerate(unique_centers) if abs(item["cy"] - ex["cy"]) <= 15), None)
        if dup_idx is None:
            unique_centers.append(item)
        elif item["conf"] > unique_centers[dup_idx]["conf"]:
            unique_centers[dup_idx] = item
    
    return [item["cy"] for item in unique_centers]

def create_row_boundaries(date_centers_y, horizontal_lines, image_height):
    if not date_centers_y:
        return []
    boundaries = []
    above = [y for y in horizontal_lines if y < date_centers_y[0]]
    boundaries.append(max(above) if above else max(0, date_centers_y[0] - 30))
    for i in range(len(date_centers_y) - 1):
        midpoint = int(round((date_centers_y[i] + date_centers_y[i + 1]) / 2))
    ...
    below = [y for y in horizontal_lines if y > date_centers_y[-1]]
    boundaries.append(min(below) if below else min(image_height - 1, date_centers_y[-1] + 30))
    ...
    if len(cleaned) != len(date_centers_y) + 1:
        cleaned = [max(0, date_centers_y[0] - 30)] + [int((date_centers_y[i] + date_centers_y[i+1])/2) for i in range(len(date_centers_y)-1)] + [min(image_height - 1, date_centers_y[-1] + 30)]
    return cleaned

def ocr_cell(crop, cell_type):
    if crop is None or crop.shape[0] < 5 or crop.shape[1] < 5:
        return ""
    pad_y, pad_x = max(2, int(crop.shape[0] * 0.05)), max(2, int(crop.shape[1] * 0.02))
    if crop.shape[0] - 2*pad_y < 5 or crop.shape[1] - 2*pad_x < 5:
        return ""
    prep = cv2.resize(crop[pad_y:-pad_y, pad_x:-pad_x], None, fx=3, fy=3, interpolation=cv2.INTER_CUBIC)
...
# ------------------------------------------------------------
# TAB 1: TIMESHEET OCR EXTRACTION
# ------------------------------------------------------------
with tab1:
    st.header("Automated Timesheet Processor")
    st.write("Upload a scanned PDF timesheet to extract all pages into structured Excel reports.")

    uploaded_file = st.file_uploader("Choose a PDF file", type="pdf")

    if uploaded_file is not None:
        if st.button("Extract Data from PDF"):
            with st.spinner("Processing document across all pages..."):
                try:
                    ...
                    for page_num in range(1, total_pages + 1):
                        ...
                        date_centers_y = find_row_centers(gray, x_bounds)
                        row_bounds = create_row_boundaries(date_centers_y, horizontal_lines, h)
                        
                        for row_idx in range(len(date_centers_y)):
                            y_top = row_bounds[row_idx]
                            y_bottom = row_bounds[row_idx+1]
                            
                            cells = []
                            for i in range(8):
                                if i+1 >= len(x_bounds):
                                    break
                                x1, x2 = max(0, x_bounds[i]), min(w, x_bounds[i+1])
                                y1, y2 = max(0, y_top), min(h, y_bottom)
                                cells.append(gray[y1:y2, x1:x2] if x2 > x1 and y2 > y1 else None)
                    ...
                    # Formatting Sheet 1: Detailed Timesheet
